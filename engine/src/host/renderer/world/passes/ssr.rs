...
pub(crate) fn render_ssr_pass(encoder: &mut wgpu::CommandEncoder, context: &WorldGpuContext) {
    let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
        label: Some("SSR Pass"),
        color_attachments: &[Some(wgpu::RenderPassColorAttachment {
            view: &context.descriptors.texture_views.ssr.reflections,
            resolve_target: None,
            ops: wgpu::Operations {
    ...
    render_pass.set_pipeline(&context.pipelines.ssr.render_pipeline);
    ...
    render_pass.set_bind_group(2, &context.descriptors.bind_groups.ssr.0, &[]);
