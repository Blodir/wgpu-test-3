...
fn get_instances(model: &modelfile::Model, transform: &Matrix4<f32>) -> Vec<Instance> {
    model
        .primitives
        .iter()
        .flat_map(|prim| {
            prim.instances
                .iter()
                .map(|inst| Instance::from_transform(transform * Matrix4::from(*inst)))
...
impl ModelData {
    pub fn update_instance_buffer(&self, queue: &wgpu::Queue, transform: &Matrix4<f32>) {
        let instances: Vec<Instance> = get_instances(&self.json, &transform);
...
impl RenderResources {
    ...
    pub fn load_model(
        ...
        let instances: Vec<Instance> = get_instances(&model, &Matrix4::identity());
        let instance_buffer =
            wgpu_context
                .device
                .create_buffer_init(&wgpu::util::BufferInitDescriptor {
