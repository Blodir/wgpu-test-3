...
use crate::{dds::dds_format_to_wgpu, renderer::texture2};
...
impl EnvironmentMapBinding {
    ...
    fn decompress_bc6h_dds_cube(dds: &ddsfile::Dds) -> Vec<Vec<u8>> {
        let mut output = vec![];
        ...
        for layer in 0..layers {
            for mip in 0..mips {
                ...
                let mip_size = (block_width * block_height * 16) as usize;
                let slice = &dds.data[current_offset..current_offset + mip_size];

                let required_f16_count = (width * height * 3) as usize;
                let mut decompressed_block =
                    vec![block_compression::half::f16::from_bits(0u16); required_f16_count];
                block_compression::decode::decode_block_bc6h(
                    slice,
                    &mut decompressed_block,
                    (width * 3) as usize,
                    false,
                );
                output.push(bytemuck::cast_slice(&decompressed_block).to_vec());
    ...
    fn load_dds_cubemap(path: &str) -> (u32, u32, u32, Vec<Vec<u8>>) {
        ...
        assert_eq!(dds.get_dxgi_format().unwrap(), DxgiFormat::BC6H_UF16);
        assert!(dds
            .header10
            .as_ref()
            .unwrap()
        ...
        let data = Self::decompress_bc6h_dds_cube(&dds);
    ...
    fn upload_bc6h_cube(device: &wgpu::Device, queue: &wgpu::Queue, dds: &Dds) -> wgpu::Texture {
        let base_width = dds.get_width();
        let base_height = dds.get_height();
        let mips = dds.get_num_mipmap_levels();
        let layers = 6;
        ...
        for layer in 0..layers {
            for mip in 0..mips {
                ...
                let padded_row_size = Self::align_to_256(row_size as usize);
                let mip_size = row_size * block_height;

                let mip_data = &dds.data[current_offset..current_offset + mip_size];
    ...
    pub fn from_image(
        ...
        let (env_map_view, env_map_sampler) = {
            let dds = Self::load_dds_raw("assets/kloofendal_overcast_puresky_8k.prefiltered.dds");
            let format = dds_format_to_wgpu(
                dds.get_dxgi_format()
                    .expect("Dds doesn't have a DXGI format."),
            );
            let texture = texture2::upload_texture(
                &dds.data,
                dds.get_width(),
                dds.get_height(),
                dds.get_num_mipmap_levels(),
        ...
        let (di_view, di_sampler) = {
            let dds = Self::load_dds_raw("assets/kloofendal_overcast_puresky_8k.di.dds");
            let format = dds_format_to_wgpu(
                dds.get_dxgi_format()
                    .expect("Dds doesn't have a DXGI format."),
            );
