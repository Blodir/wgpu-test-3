...
pub struct PipelineCache {
    pipelines: HashMap<PipelineConfig, (Rc<wgpu::RenderPipeline>, Rc<Vec<wgpu::BindGroupLayout>>)>,
}
...
impl PipelineCache {
    ...
    ) -> (Rc<wgpu::RenderPipeline>, Rc<Vec<wgpu::BindGroupLayout>>) {
        self.pipelines.entry(config.clone()).or_insert_with(|| {
            create_pipeline(config, global_bind_group_layouts, device, surface_config, shader_cache)
        }).clone()
    }
}
...
fn create_pipeline(
    ...
    let bind_group_layouts: Rc<Vec<wgpu::BindGroupLayout>> = Rc::new(
        global_bind_group_layouts.iter().chain(config.bind_group_layouts.iter()).map(|layout| {
