...
impl RenderResources {
    ...
    const PREFILTERED_PLACEHOLDER: &str = "PREFILTERED_PLACEHOLDER";
    const DI_PLACEHOLDER: &str = "DI_PLACEHOLDER";
    const BRDF_PLACEHOLDER: &str = "BRDF_PLACEHOLDER";
    ...
    fn initialize_placeholder_textures(
        ...
        let prefiltered = SampledTexture {
            view: prefiltered_texture.create_view(&TextureViewDescriptor {
                dimension: Some(wgpu::TextureViewDimension::Cube),
                ..TextureViewDescriptor::default()
            }),
            sampler: wgpu_context.device.create_sampler(&wgpu::SamplerDescriptor::default())
        };
        let di = SampledTexture {
            view: di_texture.create_view(&TextureViewDescriptor {
                dimension: Some(wgpu::TextureViewDimension::Cube),
                ..TextureViewDescriptor::default()
            }),
            sampler: wgpu_context.device.create_sampler(&wgpu::SamplerDescriptor::default())
        };
        let brdf = SampledTexture {
            view: brdf_texture.create_view(&TextureViewDescriptor::default()),
            sampler: wgpu_context.device.create_sampler(&wgpu::SamplerDescriptor::default())
        };
        ...
        self.sampled_textures.insert(Self::get_prefiltered_placeholder(), prefiltered);
        self.sampled_textures.insert(Self::get_di_placeholder(), di);
        self.sampled_textures.insert(Self::get_brdf_placeholder(), brdf);
    }
    ...
    pub fn preload_environment_map_textures(
        &mut self,
        wgpu_context: &WgpuContext,
        ...
        self.sampled_textures
            .insert(prefiltered_handle.clone(), SampledTexture { view: prefiltered_view, sampler: prefiltered_sampler });
        self.sampled_textures
            .insert(di_handle.clone(), SampledTexture { view: di_view, sampler: di_sampler });
        self.sampled_textures
            .insert(brdf_handle.clone(), SampledTexture { view: brdf_view, sampler: brdf_sampler });
    }
    ...
    pub fn get_prefiltered_placeholder() -> TextureHandle {
        TextureHandle(Self::PREFILTERED_PLACEHOLDER.to_string())
    }

    pub fn get_di_placeholder() -> TextureHandle {
        TextureHandle(Self::DI_PLACEHOLDER.to_string())
    }

    pub fn get_brdf_placeholder() -> TextureHandle {
        TextureHandle(Self::BRDF_PLACEHOLDER.to_string())
    }
    ...
    pub fn load_material(
        ...
        let binding = MaterialBinding::upload(&mat, self, &layout, wgpu_context);
    ...
    pub fn preload_model(
        ...
        let materials = {
            ...
            for mat in &model.material_paths {
                mats.push(self.load_material(mat, wgpu_context, material_layout)?);
            }
            mats
        };
